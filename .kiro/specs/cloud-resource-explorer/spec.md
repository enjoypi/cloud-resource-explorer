...
### User Story 4 - 阿里云 CloudSSO 多账号 (Priority: P3)

运维人员通过 CloudSSO 登录，系统自动获取资源目录成员账号并采集所有账号资源。

**Why this priority**: 企业级多账号管理需求

**Independent Test**: CloudSSO 登录后执行采集，验证采集到多个成员账号资源

**Acceptance Scenarios**:

1. **Given** CloudSSO 登录成功，**When** 执行采集，**Then** 获取资源目录所有成员账号
2. **Given** 成员账号列表，**When** 采集资源，**Then** 使用 AssumeRole 切换账号采集
3. **Given** 采集完成，**When** 查看结果，**Then** 资源记录包含成员账号 ID 和名称
...
- **FR-006**: 系统 MUST 按 Profile/Type/Region 粒度缓存采集结果
